...
class Trainer():
    ...
    def load_data(self):
        ...
        train_dt = self.train_dt.reshape(-1, 1)
        ...
        val_dt = self.val_dt.reshape(-1, 1)
        ...
        print('mean of train_dt:', mean_train_dt.item())
        print('mean of val_dt:', torch.mean(val_dt).item())

        self.train_dt = train_dt / mean_train_dt
        self.val_dt = val_dt / mean_train_dt
        train_dt = train_dt / mean_train_dt
        val_dt = val_dt / mean_train_dt

        self.mean_dt = torch.mean(val_dt)

        self.M_equilibrium = torch.load(f'../raw_data/L{args.L}_MC32000_h{args.h}_T{args.T:.2f}/M_equilibrium.pt')
        self.M_equilibrium = self.M_equilibrium.detach().cpu().numpy() 
        ...
        titles = ["Magnetization", "Domain Wall Density"]

        for idx in range(2):
            axes = fig.add_subplot(1, 2, idx+1)
            for i in range(plot.shape[0]):
                axes.plot(t, plot[i, :, idx])
                # if i > 10:
                #     break
            axes.set_xlabel("time (t)", fontsize=20)
            axes.set_ylabel(f'{titles[idx]}', fontsize=20)
            if idx == 0:
                axes.set_ylim(-1, 1)
            else:
                axes.set_ylim(0, 1)
        ...
        mag_susceptibility = cal_mag_susceptibility(self.M_equilibrium, args.L)
        print(f'Magnetization susceptibility: {mag_susceptibility}')
        fig = plt.figure(figsize=(8, 6))
        axes = fig.add_subplot(1, 1, 1)
        sns.kdeplot(self.M_equilibrium, fill=False, bw_adjust=2.5, cut=0)
        plt.xlim(-1, 1) 
        plt.xlabel("Magnetization", fontsize=20)
        plt.ylabel("Density", fontsize=20)
        plt.title(f"Magnetization distribution", fontsize=24)
        plt.grid(True)
        plot_name = os.path.join(self.folder,f'magnetization_distribution_true.png')
        plt.tight_layout()
        plt.savefig(plot_name)
        plt.close()  
    ...
    def plot_tra(self, epoch):
        ...
        with torch.no_grad():
            ...
            initial = self.z0_train[:, 0]
            predict_tra = self.model.predict(initial, 32000, dt=self.mean_dt)
            predict_tra = predict_tra.detach().cpu().numpy()
            titles = ["Magnetization", "Domain Wall Density"]

            t = np.arange(predict_tra.shape[1])
            fig = plt.figure(figsize=(48, 6))
            for idx in range(2):
                axes = fig.add_subplot(1, 2, idx+1)
            ...
            M = predict_tra[:, (predict_tra.shape[1]//2):, 0].flatten()  # Magnetization at the second half of the trajector
            M = np.abs(M)
            ...
            sns.kdeplot(self.M_equilibrium, fill=False, bw_adjust=2.5, cut=0, color='red', label='True')
            plt.legend()
            ...
            plt.title(f"Magnetization distribution", fontsize=24)
            plt.grid(True)
            plot_name = os.path.join(self.folder,f'magnetization_distribution_{epoch}.png')
            plt.tight_layout()
            plt.savefig(plot_name)
            plt.close()  
