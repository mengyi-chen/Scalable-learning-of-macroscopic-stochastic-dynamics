...
def cal_binder_cumulant(M):
    """Calculate Binder cumulant from magnetization data.
    ...
    Returns:
        float: Calculated Binder cumulant.
    """
    M = np.abs(M)
...
class PartialEvolutionScheme():
    ...
    def generate(self, data, patch_L):
        ...
        for i in tqdm(range(data.shape[0])):
            ...
            X1_train_partial.append(X1_partial.detach().cpu())
            idx_train_partial.append(idx.detach().cpu())
            time_step_train.append(glauber_time.detach().cpu())
...
def LocalRelax(
    ...
    for i in starts:
        for j in starts:
            x_indices = torch.arange(i - 1, i + patch_L + 1, device=device) % L
            y_indices = torch.arange(j - 1, j + patch_L + 1, device=device) % L
            
            # Extract patches for all batches using advanced indexing
            patch_spins = spins[:, x_indices[:, None], y_indices[None, :]]      
            
            updated_patches = glauber_continuous_ghost(
                patch_spins, beta=beta, h=h, n_events=n_events
            )
            
            # Handle periodic boundary conditions when writing back
            inner_rows = torch.arange(i, i + patch_L, device=device) % L
            inner_cols = torch.arange(j, j + patch_L, device=device) % L
            spins[:, inner_rows[:, None], inner_cols[None, :]] = updated_patches
    return spins
