...
parser.add_argument('--mode', default='constant_diagonal', type=str, choices=['constant_diagonal', 'diagonal', 'constant', 'arbitrary'], help='Diffusion mode')
...
parser.add_argument('--method', default='ours', type=str, choices=['ours', 'naive'], help='Training method')
...
class Trainer():
    def __init__(self, args):
        ...
        logging.basicConfig(filename=self.log_path,
                    ...
                    datefmt='%H:%M:%S',
                    level=logging.INFO)
        logging.info("Training log for Curie-Weiss model")
    ...
    def load_data(self):
        ...
        z0_train = self.z0_train.reshape(-1, 1)
        z1_train = self.z1_train.reshape(-1, 1)
        ...
        val_data_path = f'../raw_data/L{args.L}_MC200_h{args.h}_T{args.T:.2f}'
        self.z0_val = torch.load(f'{val_data_path}/z0_val.pt', map_location=self.device) # [n_tra, length_per_tra, n_dim]
        self.z1_val = torch.load(f'{val_data_path}/z1_val.pt', map_location=self.device) # [n_tra, length_per_tra, n_dim]  
        self.val_dt = torch.load(f'{val_data_path}/time_step_val.pt', map_location=self.device)

        z0_val = self.z0_val.reshape(-1, 1)
        z1_val = self.z1_val.reshape(-1, 1)
        ...
        axes.set_ylabel('Magnetization', fontsize=20)
        axes.set_ylim(-1, 1)
        plt.grid()
    ...
    def plot_tra(self, epoch):
        ...
        with torch.no_grad():
            ...
            initial = self.z0_val[:, 0].reshape(-1, 1)
            predict_tra = self.model.predict(initial, 200, dt=self.mean_dt)
