...
if __name__ == "__main__":
    ...
    print(f'Generating large system configurations (n=64^2) from small system configurations (n_s={args.patch_L}^2) ...')
    assert args.patch_L in [8, 16, 32, 64], "patch_L must be in [8, 16, 32, 64]"
    assert args.L == 64, "Currently only support L = 64"
    ...
    for iter in range(n_iters):
        ...
        for _ in tqdm(range(args.N_target_train // args.batch_size)):
            ...
            config_large = UpSample(config_small, repeat=2) # [batch_size, L, L, L]
            config_large = LocalRelax(config_large, args.T, h=args.h)
        ...
        if iter == n_iters - 1:
            for _ in tqdm(range(args.N_target_train // args.batch_size)):
                ...
                for i in range(args.batch_size):
                    ...
                    num_up = int((1 + initial_mag) * args.L ** 2 / 2)
                    num_down = args.L ** 2 - num_up
                    spins = np.concatenate([np.ones(num_up), -np.ones(num_down)])
                    np.random.shuffle(spins)
                    config_large.append(spins.reshape(args.L, args.L))
                config_large = torch.tensor(config_large, device=device, dtype=torch.float32) # [batch_size, L, L]
                X0_train.append(config_large)
        ...
        if iter < n_iters - 1:
            X_train_small = X0_train.flatten(0, 1) # [N * batch_size, L, L]
    ...
    torch.save(X_train_large, os.path.join(rootpath, 'X0_train.pt'))
    ...
    print('='*20, f'Processing patch_L: {args.patch_L}', '='*20)
    z0_train, z1_train_partial, time_step_train, z0_train_naive, z1_train_naive, idx_train_partial = generator.generate(X0_train, args.patch_L)
    
    print('z0_train shape:', z0_train.shape)
    print('z1_train shape:', z1_train_partial.shape)
    ...
    torch.save(z0_train, os.path.join(rootpath, 'z0_train.pt'))
    torch.save(z1_train_partial, os.path.join(rootpath, 'z1_train_partial.pt'))
    torch.save(z0_train_naive, os.path.join(rootpath, 'z0_train_naive.pt'))
    torch.save(z1_train_naive, os.path.join(rootpath, 'z1_train_naive.pt'))
