...
parser.add_argument('--N_atoms', default=65536, type=int)
...
class Trainer():
    def __init__(self, args):
        ...
        self.plot_T_list = [2000]
        # self.plot_T_list = [1000, 1200, 1600, 1800, 2400, 3000]
        
        start_message = f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}: Initializing."
        print(start_message)
        date = datetime.now().strftime('%Y_%m_%d_%H:%M:%S')
        self.folder = os.path.join('../checkpoints_partial',f'model_atoms_{args.N_atoms}_{args.model_name}_{date}')
        ...
        if args.model_name == 'S_OnsagerNet':
            ...
            self.model = S_OnsagerNet(self.dt, mode=args.mode, n_dim=args.hidden_dim, epsilon=args.epsilon, embedding_dim=args.embedding_dim, forcing=False).to(self.device)
        elif args.model_name == 'SDE_Net':
            self.model = SDE_Net(self.dt, mode=args.mode, n_dim=args.hidden_dim, epsilon=args.epsilon, embedding_dim=args.embedding_dim).to(self.device)
        else:
    ...
    def load_data(self):
        ...
        # # FIXME: only use T >= 1400K data for training
        # idx = torch.where(self.train_T[:, 0] <=1000)[0]
        # self.z0_train = self.z0_train[idx]
        # self.z1_train_partial = self.z1_train_partial[idx]
        # self.time_step = self.time_step[idx]
        # self.train_T = self.train_T[idx]
        ...
        # print('self.plot_data shape:', self.plot_data.shape) # [20, 500, 12]
        ...
        for T in self.plot_T_list:
            # index = torch.unique(torch.where(self.val_T == T)[0])
            index = torch.unique(torch.where(self.plot_T == T)[0])
            fig = plt.figure(figsize=(48, 6))
            # plot = self.val_data[index].detach().cpu().numpy() # [n_tra, length_per_tra, hidden_dim]
            plot = self.plot_data[index].detach().cpu().numpy() # [n_tra, length_per_tra, hidden_dim]
            # true_t = self.val_time[index].detach().cpu().numpy() # [n_tra, length_per_tra]
