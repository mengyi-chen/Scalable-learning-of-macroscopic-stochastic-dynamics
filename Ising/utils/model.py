...
class Encoder(nn.Module):
    def __init__(self, hidden_dim=2, macro_dim=6, L=64, box_L=16, h=0.0):
        super().__init__()
        assert box_L in [8, 16, 32, 64]
    ...
    def encode(self, x0, x1, partial=False, index=None):
        ...
        if partial == True:
            ...
            z0_hat_partial = self.model(x0_partial) # [B-1, 1, 16, 16, 16] -> [B-1, hidden_dim]
            z1_hat_partial = self.model(x1_partial) # [B-1, 1, 16, 16, 16] -> [B-1, hidden_dim]

            z0_partial = torch.cat([z0_macro_partial, z0_hat_partial], -1) # [B-1, macro_dim + hidden_dim]
            z1_partial = torch.cat([z1_macro_partial, z1_hat_partial], -1) # [B-1, macro_dim + hidden_dim]
            ...
            z0_naive = z0
            z1_naive = z1_partial 
            z1_naive = (z1_naive - self.min_val) / (self.max_val - self.min_val)
...
class Decoder(nn.Module):
    def __init__(self, hidden_dim=2, macro_dim=6, L=64, box_L=16): # Added L
        super().__init__()
        assert L % box_L == 0, "L must be divisible by box_L"
