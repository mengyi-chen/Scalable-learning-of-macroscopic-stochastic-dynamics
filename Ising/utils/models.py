...
class Encoder(nn.Module):
    def __init__(self, closure_dim=2, macro_dim=2, L=64, patch_L=16):
        ...
        # Since we consider different patch sizes (8^2, 16^2, 32^2, 64^2), we define the encoder dynamically for different patch sizes
        # Calculate number of downsampling layers for patch size = patch_L * patch_L
        if patch_L == 64 or patch_L == 32:
            num_downsamples = 3
        else:
...
class Decoder(nn.Module):
    def __init__(self, closure_dim=2, macro_dim=2, L=64, patch_L=16): # Added L
        ...
        # Since we consider different patch sizes (8^2, 16^2, 32^2, 64^2), we define the decoder dynamically for different patch sizes
        # Calculate number of upsampling layers needed to reach patch size = patch_L * patch_L from initial 4x4
