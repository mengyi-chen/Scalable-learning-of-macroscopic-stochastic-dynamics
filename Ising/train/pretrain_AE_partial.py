...
parser.add_argument('--T', default=2.50, type=float)
...
class Trainer():
    def __init__(self, args):
        ...
        self.data_path_val = f'../raw_data/L{args.L}_MC500_h{args.h}_T{args.T:.2f}'
        self.save_path = f'../raw_data_upscale/scaleup_box_L_{args.box_L}_L{args.L}_h{args.h}_T{args.T:.2f}'

        start_message = f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}: Initializing."
        print(start_message)

        date = datetime.now().strftime('%Y_%m_%d_%H:%M:%S')
        self.folder = os.path.join(f'../checkpoints',f'AE_{args.Task_NAME}_epoch_{args.epoch}_box_L_{args.box_L}_L_{args.L}_{date}')
    ...
    def process(self):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(self.X0_train.shape[0])):
                ...
                z0, z1_partial, z0_naive, z1_naive = self.model.encode(X0, X1_partial, partial=True, index=idx_partial)
            ...
            print('z0_train_naive shape:', z0_train_naive.shape) # [n_tra, length_per_tra, macro_dim + hidden_dim]
            print('z1_train_naive shape:', z1_train_naive.shape) # [n_tra, length_per_tra, macro_dim + hidden_dim]
